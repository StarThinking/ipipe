...
int ipipe_switch_domain(unsigned int irq)
{
        printk("[msx] ipipe_switch_domain, irq = %u\n", irq);
        
        int ret = 0;
        struct ipipe_domain *rtai_domain, *linux_domain;
        rtai_domain = ipipe_head_domain;
        linux_domain = ipipe_root_domain;

        ipipe_irq_handler_t handler = linux_domain->irqs[irq].handler;
        void *cookie = linux_domain->irqs[irq].cookie;
        ipipe_irq_ackfn_t ackfn = linux_domain->irqs[irq].ackfn;
       
        ipipe_free_irq(linux_domain, irq);
        ipipe_request_irq(rtai_domain, irq, handler, cookie, ackfn);

        return 0;
}
EXPORT_SYMBOL_GPL(ipipe_switch_domain);

int ipipe_switch_domain_back(unsigned int irq)
{
        printk("[msx] ipipe_switch_domain_back, irq = %u\n", irq);
        ...
        ipipe_irq_handler_t handler = rtai_domain->irqs[irq].handler;
        void *cookie = rtai_domain->irqs[irq].cookie;
        ipipe_irq_ackfn_t ackfn = rtai_domain->irqs[irq].ackfn;

        ipipe_free_irq(rtai_domain, irq);
        ipipe_request_irq(linux_domain, irq, handler, cookie, ackfn);
        
        return 0;
}
EXPORT_SYMBOL_GPL(ipipe_switch_domain_back);
